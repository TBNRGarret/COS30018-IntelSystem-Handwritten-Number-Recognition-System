...
def binarize_otsu(gray: np.ndarray, invert: bool = True) -> np.ndarray:
    """
    Produce a clean binary mask using Gaussian blur + Otsu's global threshold.

    Why Gaussian blur first:
        Noise causes Otsu to compute a sub-optimal threshold. A 5×5 blur
        smooths out high-frequency noise while preserving stroke edges.

    Why invert=True by default:
        Real-world images typically have dark digits on a light background.
        After inversion the binary mask matches MNIST convention:
        digit pixels = 255 (white), background = 0 (black).

    Args:
        gray:   Grayscale image  (H, W)  uint8
        invert: Flip foreground / background so digits become white.
...
def preprocess(image: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    Task 1 preprocessing pipeline.
        ...
        BGR image  →  Grayscale  →  Otsu binary mask

    Returns both so Task 2 can:
        • run CCA on `binary`
        • crop digit patches from `gray` (needed for smart resize in Task 2)
    ...
    binary = binarize_otsu(gray, invert=True)
    return gray, binary
...
def compare_binarization_techniques(image_path: str, save: bool = True):
    """
    Side-by-side comparison of all four investigated pixel-value representations.
    Use for the Task 1 section of the project report.

    Shows why Otsu's binarization is the correct choice for CCA input.
...
def show_full_pipeline(image_path: str, save: bool = True):
    ...
    Visualise the complete Task 1 pipeline on a real image:
        Original → Grayscale → Otsu Binary
    ...
    # Annotate arrow flow
    fig.text(0.345, 0.5, "→", fontsize=28, ha="center", va="center")
    fig.text(0.655, 0.5, "→", fontsize=28, ha="center", va="center")
